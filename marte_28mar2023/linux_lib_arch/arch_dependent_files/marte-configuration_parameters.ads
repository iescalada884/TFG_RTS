...
package MaRTE.Configuration_Parameters is
   ...
   type Supported_Architectures is (ARCH_X86, ARCH_LINUX, ARCH_LINUX_LIB,
                                    ARCH_XTRATUM, ARCH_RPI);
   subtype MaRTE_Architecture is Supported_Architectures
     range ARCH_LINUX_LIB .. ARCH_LINUX_LIB; --  DO NOT EDIT !!
   --  To change architecture run 'msetcurrentarch'.

   ----------------------------------------------------------------------------
   --  Preallocated Resources  ------------------------------------------------
   ----------------------------------------------------------------------------
   subtype Preallocated_Resources is Boolean range False .. False;
   ...
   ----------------------------------------------------------------------------
   Num_User_Tasks_Mx : constant := 0;  --  {PREALLOCATED}
   ...
   Num_MaRTE_Tasks_Mx : constant := 0;  --  {PREALLOCATED}
   ...
   Num_Timed_Events_Initial : constant := 2000;
   ...
   Queued_Signals_Maximum : constant := 0;  --  {PREALLOCATED} 32
   --  Never below 32 because a POSIX limit.

   type Keys_Range is range 1 .. 40;
   ...
   --  Stack Size  ------------------------------------------------------------
   ----------------------------------------------------------------------------
   Stack_Size_In_Bytes : constant := 50 * 1024;
   --  Every task shall have a stack of this size.

   Main_Stack_Size_In_Bytes : constant := 1;
   --  Size of the main task's stack (only used in x86 architecture).

   Signal_Handler_Stack_Size_In_Bytes : constant := 20 * 1024;
   --  Size of the signal handler task stack

   Idle_Stack_Size_In_Bytes : constant := 20 * 1024;
   ...
   ----------------------------------------------------------------------------
   --
   --  These parameters are not used in this architecture. Dynamic
   --  memory functions (malloc(), free(), etc.) are provided by the
   --  Linux standard C library.
   subtype Use_TLSF_Memory_Allocator is Boolean range False .. False;
   ...
   --  Size of the dynamic memory pool.
   ...
   Round_Robin_Interval_Time_Period_NS : constant := 30_000_000; -- nanoseconds
   Round_Robin_Interval_Time_Period_S : constant :=
     Float (Round_Robin_Interval_Time_Period_NS) / 1.0E9;
   ...
   --  Enable or disable the group CPU-time clocks and timers. To use this
   --  functionality 'Use_CPU_Time_Clocks_And_Timers' and
   --  'Use_Task_Sets' have to be set to true.

   subtype Account_Interrupt_Handlers_Time_Separately
      is Boolean range True .. True;
   --  When this parameter is true execution time consumed by the
   --  interrupt handlers is not charged to the execution time clock
   --  of any task. This time is charged to a different clock
   --  (CLOCK_INTERRUPTS_CPUTIME) that can be read using the POSIX
   --  clocks interface (clock_gettime) or the Ada package
   --  Ada.Execution_Time.Interrupts.

   subtype Measure_Interrupt_Handlers_Time is Boolean range True .. True;
   --  When this parameter is true execution time consumed by the
   --  interrupt handlers is measured by the system even if
   --  Account_Interrupt_Handlers_Time_Separately is false. The time
   --  is charged to CLOCK_INTERRUPTS_CPUTIME like explained above. If
   --  Account_Interrupt_Handlers_Time_Separately is true, the value
   --  of this parameter is not considered and
   --  CLOCK_INTERRUPTS_CPUTIME is always available.
   ...
   ----------------------------------------------------------------------------
   subtype Use_HW_Interrupts_Control is Boolean range False .. False;
   ...
   Mx_Number_Of_Interrupt_Associations : constant := 0;  --  {PREALLOCATED} 4
