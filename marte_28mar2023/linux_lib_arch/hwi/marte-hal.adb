...
package body MaRTE.HAL is
   ...
   HWT_HZ : HWTime := 1_000_000_000;
   --  in this architecture HWTime is a count of nanoseconds
   ...
   begin
      ...
      Next_Activation := Get_HWTime + Interval;
   end Program_Timer;
   ...
   begin
      Result := Get_Time_Of_Day (TV'Access, null);
      pragma Assert (Result = 0);

      T := HWTime (TV.tv_sec) * NS_Per_S + HWTime (TV.tv_usec) * 10#1#E3;
      pragma Assert (T >= Last_HWTime_Read);
      Last_HWTime_Read := T;
   ...
   procedure Initialize is
      ...
      procedure Linux_Signals_Init; --
      pragma Import (C, Linux_Signals_Init,
                       "linux_signals_init");  --  'boot/linux_signals.c'
   begin
      pragma Assert (Duration'Small = 1.0/1e9);
      --  We rely that Duration is an exact count of nanoseconds.

      Linux_Signals_Init;  --  No signals until everything is properly set up
      Interrupt_Tables.Inicialize;

      --  Initialize HW_Timer informing that Hwtime_Hz := 1_000_000_000
      HW_Timer_Init (1_000_000_000);
   end Initialize;
