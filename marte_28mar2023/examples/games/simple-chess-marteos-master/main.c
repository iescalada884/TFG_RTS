...
--                                                                         --
--                                                                         --
--  The heart of this program (the artificial intelligence) comes from     --
--  the open source command-line program mscp by Marcel van Kervinck:      --
--  http://marcelk.net/mscp/                                               --
--  I've ported it to MaRTE OS, made some small modifications and created  --
--  a graphical interface based on the one I made for EtherChess.          --
--                                                                         --
-----------------------------------------------------------------------------
--                               License                                  -- 
-----------------------------------------------------------------------------
...
// Original header info from mscp.c
/*----------------------------------------------------------------------+
 |                                                                      |
...
/*----------------------------------------------------------------------+
 |      qsort  - MARTE OS version                                       |
 +----------------------------------------------------------------------*/

//reference: http://en.wikibooks.org/wiki/Algorithm_Implementation/Sorting/Quicksort#C
...
// MaRTE OS without a file system -> can't load the book.txt info from a file
static void load_book(char *filename) // TODO: just added the bare minimum, should try to add the rest of the book
{
        
        char line[128]="e4 c5 Nf3 d6 d4 cxd4 Nxd4 Nf6 Nc3 a6 Bg5 e6 f4 Qb6 Qd2 Qxb2 Rb1";
        char *s;
        int                     num, move;
...
/*static char startup_message[] =
        "\n"
        "This is a port of MSCP 1.4 (Marcel's Simple Chess Program) for MaRTE OS\n"
        "\n"
        "Original code by: Marcel van Kervinck\n"
        "Ported to MaRTE OS by: Alvaro Garcia\n"
        "\n"
...
int guiVersion(void) // TODO: not finished -> add messages (your turn, check mate...)
{
        int i;
        int cmd;
        char line[128];
        ...
        draw_board(backBuffer,screenWidth,20,20);
        draw_pieces(backBuffer,screenWidth,board,20,20);
        ...
        for (;;) { /* main loop */
                ...
                ////////////////////    
                        ...
                        char * textBoard[64]={  "a8","b8","c8","d8","e8","f8","g8","h8",
                        ...
                        while(!validMovement){
                            ...
                            draw_board(backBuffer,screenWidth,20,20);
                            draw_pieces(backBuffer,screenWidth,board,20,20);
                            draw_cursor(backBuffer,screenWidth,20,20,cursorX,cursorY);
                            
                            bmpfont_print(theString, 200, 10, 60, backBuffer);
                                                        
                            if(pieceSelected){
                                draw_selected(backBuffer,screenWidth,20,20,x1,y1);
                            }
                            blit(backBuffer,screenWidth,screenHeight);//swap buffers
                        ...
                        //sprintf(theString,"%s%s",textBoard[y1*8+x1],textBoard[y2*8+x2]);
                        strcat ( theString, textBoard[y1*8+x1]);
                        strcat ( theString, textBoard[y2*8+x2]);
                        /*theString[0]="h";
                        theString[1]=textBoard[y1*8+x1][1];
                        theString[2]=textBoard[y2*8+x2][0];
                        theString[3]=textBoard[y2*8+x2][1];
                        
                        sprintf(theString,"h2h4");*/
                        
                ///////////////   

                /*if (1 != sscanf(theString, "%s", name)) continue;
                ...
                move = parse_move(theString, &dummy);
                if (move) {
                        make_move(move);
                        hash_stack[ply] = compute_hash();
                        print_board();
                } else {
                        bmpfont_print("bad move", 200, 100, 60, backBuffer);
                }
...
{
    printf("\n");
    printf("This is a port of MSCP 1.4 (Marcel's Simple Chess Program) for MaRTE OS\n");
    printf("\n");
    printf("Original code by: Marcel van Kervinck\n");
    printf("Ported to MaRTE OS and graphical interface by: Alvaro Garcia\n");
    printf("\n");
    ...
    while(!done)
    {
        printf("\n\nChoose version (type the number and press enter):\n\n");
        printf("1: start the text version.\n");
        printf("2: start the graphical version.\n");
        printf("> ");
