...
static struct element_t new_elements;
...
int main()
{
        ...
        list_for_each_entry_safe(elem_p, tmp, &the_elements.list, list) {
                if (elem_p->num == 2) {
        ...
        INIT_LIST_HEAD(&new_elements.list);

        list_splice(&the_elements.list, &new_elements.list);

        INIT_LIST_HEAD(&the_elements.list);

        printf("traverse the list\n");
        list_for_each_entry(elem_p, &new_elements.list, list) {
                printf("%d\n", elem_p->num);
        }
