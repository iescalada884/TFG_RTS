 ...
 *
 * Simple test: arm a one shoot timer based on a group clock and make threads
 * to execute.
 *
 * Test uses quite a lot of auxiliary POSIX stuff: sigwaitinfo, siqueue, ...
 */
#include <unistd.h>
#include <stdio.h>
...
#define TEST_NAME "Timer based on Group clock test 2"
...
static void time_printf(const char *format, ...)
{
    ...
    printf("%d:", current_time.tv_sec * 1000 + current_time.tv_nsec/1000000);
