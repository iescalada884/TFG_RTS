...
/*
 * The test create several FIFO threads and allow them execute for a while,
 * then the policy of the threads is changed to round robin and allow
 * them to execute for a while. At the end it checks it the number of context
 * switches is the expected.
 *
 */
#include <stdio.h>
#include <unistd.h>
...
#define RR_THREAD_NUM 2 // create this number of RR threads
#define RR_PRIO 10
...
int main()
{
  ...
  for (i=0; i<RR_THREAD_NUM; i++) {
    CHK( pthread_create (&th[i], &attr, rr_thread, (void *) i) );
  }
  ...
  // change threads policy
  printf ("Main changes threads' policy to Round Robin\n");
  for (i=0; i<RR_THREAD_NUM; i++) {
    CHK( pthread_setschedparam (th[i], SCHED_RR, &param) );
  }
  cs = 0;
  cs_count = 0;
