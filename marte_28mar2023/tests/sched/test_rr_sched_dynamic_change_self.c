...
/*
 * The test launch several FIFO threads that change themselves their policy
 * to round robin.
 * The main thread allow them to execute for a while. At the end it checks
 * if the number of context switches is the expected.
 *
 */
#include <stdio.h>
#include <unistd.h>
...
void *rr_thread (void *arg)
{
  ...
  printf ("Thread %d changes to RR\n", id);
  CHK( pthread_setschedparam (pthread_self(), SCHED_RR, &param) );
