...
procedure Test_Group_Budgets_Handler is
...
begin
   ...
   GB.Set_Handler (Group_Budgets_Handler_PO.PO.Handler'Access);

   --  Set group budget for 2 executions
   GB.Add (RT.To_Time_Span (Task_Execution_Time*2));
   ...
   for I in 1 .. Task_Loops loop
      ...
      Reports.Assert ((not GB.Budget_Has_Expired and I <= 2)
                      or (GB.Budget_Has_Expired and I > 2));
      ...
      Reports.Assert ((not GB.Budget_Has_Expired and I <= 1)
                     or (GB.Budget_Has_Expired and I > 1));
   end loop;

   Reports.Assert (GB.Budget_Has_Expired and PO.PO.Handler_Count = 1);
   Reports.Test_OK;

end Test_Group_Budgets_Handler;
