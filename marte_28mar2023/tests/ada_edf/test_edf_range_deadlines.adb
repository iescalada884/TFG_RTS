...
--                                                             V2.0 2019-05-24
--
--              'T e s t _ E D F _ R a n g e _ D e a d l i n e s'
--
--                               Ada Program
--
--
--  File 'test_edf_range_deadlines.adb'                               By MAR.
--
--  Creates an EDF priority range. Each tasks is created with a priority
--  out of the EDF range, its deadline is set and, finally, its priority is
--  changed to a priority inside the EDF range.
--
--  The test checks the tasks executes acording to their deadlines.
--
--  ----------------------------------------------------------------------
...
--
------------------------------------------------------------------------------
pragma Priority_Specific_Dispatching (EDF_Across_Priorities, 20, 25);
...
procedure Test_EDF_Range_Deadlines is
   ...
   Main_Prio : constant := 30;
   Lower_EDF_Prio : constant := 20;
...
begin
   ...
   --  Set deadlines and change priority of tasks to convert them into EDF
   --  tasks
   for I in EDF_Tasks'Range loop
      EDF.Set_Deadline (Start_Time + Deadlines (I));
      Ada.Dynamic_Priorities.Set_Priority (Lower_EDF_Prio + I,
                                           EDF_Tasks (I)'Identity);
   end loop;
