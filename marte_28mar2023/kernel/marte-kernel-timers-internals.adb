...
package body MaRTE.Kernel.Timers.Internals is
   ...
   ---------------------------------------
   -- Interrupt_Handlers_Execution_Time --
   ---------------------------------------

   --  Time consumed for the execution by the interrupt handlers. It can be
   --  read by the application using POSIX clock CLOCK_INTERRUPT_CPUTIME or Ada
   --  package Ada.Execution_Time.Interrupts

   Interrupt_Handlers_Execution_Time : HAL.HWTime := 0;

   ----------------------------------------------
   -- Account_Interrupt_Handler_Execution_Time --
   ----------------------------------------------

   procedure Account_Interrupt_Handler_Execution_Time
     (Interval : in HAL.HWTime) is
   begin
      Interrupt_Handlers_Execution_Time := Interrupt_Handlers_Execution_Time
        + Interval;
   end Account_Interrupt_Handler_Execution_Time;
   pragma Inline (Account_Interrupt_Handler_Execution_Time);

   -------------------------------------------
   -- Get_Interrupt_Handlers_Execution_Time --
   -------------------------------------------

   function Get_Interrupt_Handlers_Execution_Time
     return HAL.HWTime is
   begin
      return Interrupt_Handlers_Execution_Time;
   end Get_Interrupt_Handlers_Execution_Time;
   pragma Inline (Get_Interrupt_Handlers_Execution_Time);
   ...
   begin
      return
        ...
        or else Clock = CLOCK_INTERRUPTS_CPUTIME
        or else ((CP.Account_Interrupt_Handlers_Time_Separately'First or else
                    CP.Measure_Interrupt_Handlers_Time'First)
                 and then Clock = CLOCK_INTERRUPTS_CPUTIME)
