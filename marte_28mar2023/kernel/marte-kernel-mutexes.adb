...
package body MaRTE.Kernel.Mutexes is
   ...
   function Pthread_Mutex_Init (M    : not null access Mutex;
      ...
      case M.Policy is
         ...
         when APPSCHED_PROTOCOL =>
            ...
            begin
               ...
               SCHD.Do_Scheduling;
               SCHD.Context_Switch_Window; -- Activate appscheduler task
   ...
   ---------------------------
   function Pthread_Mutex_Destroy (M : not null access Mutex) return Int is
      Flags : Integer;
   begin
      K.Enter_Critic_Section (Flags);
      ...
      loop -- loop until the mutex is locked
         ...
         if M.Policy = APPSCHED_PROTOCOL then
            ...
            if M.Owner /= null then  --  Mutex locked
               ...
               SCHD.Do_Scheduling;
               SCHD.Context_Switch_Window; -- leave CPU
