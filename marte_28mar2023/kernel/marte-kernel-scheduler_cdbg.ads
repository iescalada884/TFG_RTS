...
package MaRTE.Kernel.Scheduler_CDbg is
   ...
   procedure In_Timer_Handler (Time  : HAL.HWTime);

   procedure Check_Timer_Handler_Invocation_Time (CS_Time : HAL.HWTime);
   ...
   procedure Leaving_Timer_Handler;
   --  Leaves the scheduler timer handler
   ...
   procedure CPU_Time_Accounting (Time : in HAL.HWTime);

   -------------------------
   -- Task_Changes_Policy --
   -------------------------
   procedure Task_Changes_Policy (T      : in Task_Id;
                                  Policy : in K.Scheduling_Policies);
   ...
   -------------------------------------
   -- Scheduler_Timed_Event (RR y SS) --
   -------------------------------------
   procedure Enqueue_Scheduler_Timed_Event (T : in Task_Id);
