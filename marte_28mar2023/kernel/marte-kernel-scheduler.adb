...
package body MaRTE.Kernel.Scheduler is
   ...
   use type Int, HAL.HWTime, CP.Supported_Architectures;
   ...
   Round_Robin_Quantum : constant Duration :=
     CP.Round_Robin_Interval_Time_Period_S;
   ...
   --  'base_irq_nest' is defined in the HAL (in 'boot/base_irq.c' in x86
   --  arch).
   ...
   begin
      ...
      if HAL.Is_Context_Switch_Pending then         
         pragma Debug (Debug.Assert_Task_Is_Ok (Former_Task));
         return Former_Task;
   ...
   pragma Inline (Dequeue_Task_Ending_Reenqueuement_Deferred);
    ...
    -------------------------------------------
   -- Reset_To_Capacity_Sched_Timed_Event_RT --
   --------------------------------------------
   procedure Reset_To_Capacity_Sched_Timed_Event_RT (T : in Task_Id);
   procedure Reset_To_Capacity_Sched_Timed_Event_RT (T : in Task_Id) is
   begin
      pragma Debug (DBG.Assert (T = Running_Task));
      pragma Debug (DBG.Assert (T.Capacity > 0));
      pragma Debug (DBG.Assert (not Is_In_The_List
      ...
      T.Scheduler_Timed_Event.CPU_Time :=
        T.Scheduler_Timed_Event.CPU_Time + T.Capacity;
      T.Scheduler_Timed_Event.T :=
        T.Scheduler_Timed_Event.T + T.Capacity;
      T.Scheduler_Timed_Event.Task_Where_Queued := TCB_Ac (T);

      pragma Debug (CDBG.Enqueue_Scheduler_Timed_Event (T));

      CPU_Time_Timed_Events_Lists_Order.Enqueue_In_Order
        (T.Scheduler_Timed_Event'Access, T.CPU_Time_TEs_Q);
   end Reset_To_Capacity_Sched_Timed_Event_RT;

   ----------------------------------
   -- Enqueue_Sched_Timed_Event_RT --
   ----------------------------------
   procedure Enqueue_Sched_Timed_Event_RT (T : in Task_Id;
                                           Now : in HAL.HWTime);
   procedure Enqueue_Sched_Timed_Event_RT (T : in Task_Id;
   ...
   end Enqueue_Sched_Timed_Event_RT;

   -------------------------------------
   -- Enqueue_Sched_Timed_Event_No_RT --
   -------------------------------------
   procedure Enqueue_Sched_Timed_Event_No_RT (T : in Task_Id);
   procedure Enqueue_Sched_Timed_Event_No_RT (T : in Task_Id) is
   begin
      pragma Debug (DBG.Assert (T /= Running_Task));
      pragma Debug (DBG.Assert (T.Capacity > 0));
      pragma Debug(DBG.Assert (not Is_In_The_List
      ...
      CPU_Time_Timed_Events_Lists_Order.Enqueue_In_Order
        (T.Scheduler_Timed_Event'Access, T.CPU_Time_TEs_Q);
   end Enqueue_Sched_Timed_Event_No_RT;

   -----------------------------------
   -- Dequeue_Scheduler_Timed_Event --
   -----------------------------------
   procedure Dequeue_Scheduler_Timed_Event (T : in Task_Id);
   procedure Dequeue_Scheduler_Timed_Event (T : in Task_Id) is
   begin
      pragma Debug (DBG.Assert (Is_In_The_List
                    (T.Scheduler_Timed_Event'Access, T.CPU_Time_TEs_Q)));
      pragma Debug
        (DBG.Assert (T.Scheduler_Timed_Event.Task_Where_Queued = TCB_Ac (T)));

      pragma Debug (CDBG.Dequeue_Scheduler_Timed_Event (T));
      T.Scheduler_Timed_Event.Task_Where_Queued := null;
      CPU_Time_Timed_Events_Lists.Dequeue (T.Scheduler_Timed_Event'Access,
                                           T.CPU_Time_TEs_Q);
      ...
      case T.Sched_Policy is
         when SCHED_FIFO | SCHED_RR | SCHED_SPORADIC | SCHED_OTHER
            ...
            if T.Sched_Policy = SCHED_SPORADIC then
               ...
               --  finishes.
               Enqueue_Sched_Timed_Event_No_RT (T);
   ...
   begin
      ...
      case Running_Task.Sched_Policy is
         ...
         when SCHED_RR =>
            Dequeue_Scheduler_Timed_Event (Running_Task);
   ...
   begin
      ...
      pragma Debug (DBG.Assert (Task_OK (T)));
      pragma Debug (DBG.Assert (T.Status = READY));
      pragma Debug (CDBG.Ready_Task_Changes_Urgency (T)); -- {CDBG}
      
      if T = Running_Task and then T.Reenqueuement_Deferred then
   ...
   begin
      pragma Debug (DBG.Assert (Initialized));
      pragma Debug (DBG.Assert (Task_OK (T)));
      pragma Debug (CDBG.Task_Changes_Policy (T, Policy)); -- {CDBG}
      ...
      if T.Sched_Policy /= Policy then
         ...
         case T.Sched_Policy is
            ...
            when SCHED_RR | SCHED_SPORADIC =>
               if T.Scheduler_Timed_Event.Task_Where_Queued /= null then
                  pragma Debug (DBG.Assert (Is_In_The_List
                                (T.Scheduler_Timed_Event'Access,
                                   T.CPU_Time_TEs_Q)));
                  Dequeue_Scheduler_Timed_Event (T);
               else
                  pragma Debug (DBG.Assert (not Is_In_The_List
                                (T.Scheduler_Timed_Event'Access,
                                   T.CPU_Time_TEs_Q)));
         ...
         case Policy is
            ...
            when SCHED_RR | SCHED_SPORADIC =>
               ...
               if T = Running_Task then
                  Enqueue_Sched_Timed_Event_RT (T, Now);
               else
                  Enqueue_Sched_Timed_Event_No_RT (T);
               end if;
               T.Scheduler_Timed_Event.Task_Where_Queued := TCB_Ac (T);
   ...
   begin
      ...
      case Running_Task.Sched_Policy is
         when SCHED_FIFO | SCHED_SPORADIC | SCHED_OTHER =>
            null;
            
         when SCHED_EDF =>
            if Running_Task.New_Deadline_For_Next_Activation then
               --  Set new deadline

               Running_Task.Base_Urgency :=
                 K.Task_Urgency (HAL.HWTime'Last - Running_Task.Deadline);
               Running_Task.Active_Urgency := Running_Task.Base_Urgency;
               Running_Task.New_Deadline_For_Next_Activation := False;
            end if;
            
         when SCHED_RR =>
            --  Reset RR quantum
            Dequeue_Scheduler_Timed_Event (Running_Task);
            Reset_To_Capacity_Sched_Timed_Event_RT (Running_Task);
   ...
   pragma Export (C, Timer_Interrupt_Handler);
   ...
   begin
      ...
      if CP.MaRTE_Architecture'First = CP.ARCH_GNAT_BB_ARM then
         
         --  The timer activation time is managed by the GNAT BB and
      ...
      pragma Debug (CDBG.In_Timer_Handler (CS_Time)); -- {CDBG}

      if CS_Time = HAL.HWTime'Last then
         --  Spurious interrupt. The timer has not been programmed, that is
         --  the reason why the timer activation time is HWTime'Last. This
         --  happens after boot-up (if timer is not programmed before its
         --  first "spurious" expiration)
         CS_Time := HAL.Get_HWTime;
         
      else
         null;
         pragma Debug (CDBG.Check_Timer_Handler_Invocation_Time (CS_Time));
      end if;
...
--      pragma Debug (CDBG.In_Timer_Handler (CS_Time)); -- {CDBG}
      ...
      loop
         ...
         Event := TE_T.Extract_Expired_Event (Running_Task, CS_Time);
         ...
         elsif Event.All in K.Replenishment_TE.Replenishment_Timed_Event'Class
         then
      ...
      pragma Debug (CDBG.Leaving_Timer_Handler); -- {CDBG}

      K.Leave_Critic_Section_From_Interrupt;

      --  Hardware timer is reprogrammed for the next expiration by calling
      --  TE_T.Reprogram_Timer_After_Expiration from End_Interrupt
   ...
   --------------------------
   --  Shared by Do_Scheduling, Start_Interrupt and End_Interrupt
   In_Interrupt_Handler : Boolean := False;

   -------------------------
   -- CPU_Time_Accounting --
   -------------------------

   procedure CPU_Time_Accounting (Old_Task : Task_Id;
                                  New_Task : Task_Id;
                                  Now      : HAL.HWTime);
   pragma Inline (CPU_Time_Accounting);
   CPU_E : CPU_Time_Timed_Event_Ac;     --  Local to 'CPU_Time_Accounting'

   procedure CPU_Time_Accounting (Old_Task : Task_Id;
                                  New_Task : Task_Id;
                                  Now      : HAL.HWTime) is
   begin
      --  Only to be called when CPU-time accounting required
      pragma Debug (DBG.Assert
        (CP.Use_CPU_Time_Clocks_And_Timers   or else
           CP.Use_Round_Robin_Scheduling_Policy or else
           CP.Use_Sporadic_Server_Scheduling_Policy));

      pragma Debug (CDBG.CPU_Time_Accounting (Now)); -- {CDBG} 

     --  Old task CPU-time accounting

      Old_Task.Used_CPU_Time := Old_Task.Used_CPU_Time +
        (Now - Old_Task.Running_Starting_Time);
      ...
      --  read interrupt ending time (if in interrupt handler)

      if In_Interrupt_Handler then
         CS_Time := HAL.Get_HWTime;

         --  Account interrupt time

         TMRI.Account_Interrupt_Handler_Execution_Time (CS_Time - Now);
      end if;

      --  Heir task CPU-time accounting

      -- update activation time
      New_Task.Running_Starting_Time := CS_Time;

      --  Manage group clocks
      if CP.Use_Group_Clocks'First then

         pragma Debug (DBG.Assert (New_Task.Set = null or else
           TS.Task_Set_OK (New_Task.Set)));
         if New_Task.Set /= null
           and then New_Task.Set.Group_TE_Ac /= null
         then
            --  Perform actions on Group_TE_Ac
            pragma Debug (DBG.Assert (GCLK.Group_TE_OK
              (New_Task.Set.Group_TE_Ac)));

            --  Set CPU_Time of Group_TE_Ac
            New_Task.Set.Group_TE_Ac.CPU_Time :=
              New_Task.Used_CPU_Time +
                (New_Task.Set.Group_TE_Ac.Group_Expiration_Time -
                     New_Task.Set.Consumed_Time);

            if New_Task.Set.Group_TE_Ac.Armed
              and then New_Task.Set.Group_TE_Ac.Task_Where_Queued /=
                TCB_Ac (New_Task)
            then
               --  Move Group_TE_Ac from one task to another
               if
                 New_Task.Set.Group_TE_Ac.Task_Where_Queued /= null then
                  ...
                  Dequeue (New_Task.Set.Group_TE_Ac,
                           New_Task.Set.Group_TE_Ac.
                             Task_Where_Queued.CPU_Time_TEs_Q);
               end if;
               --  Enqueue in Heir's list
               Enqueue_In_Order (New_Task.Set.Group_TE_Ac,
                                 New_Task.CPU_Time_TEs_Q);
               New_Task.Set.Group_TE_Ac.Task_Where_Queued :=
                 TCB_Ac (New_Task);

               if New_Task.Set.Group_TE_Ac.all in
                 TH.Timed_Handler_Timed_Event'Class
               then
                  TH.Timed_Handler_TE_Ac
                    (New_Task.Set.Group_TE_Ac).Queued := True;
      ...
      CPU_E := Head (New_Task.CPU_Time_TEs_Q);
      while CPU_E /= null loop
         ...
         CPU_E.T := CS_Time + (CPU_E.CPU_Time - New_Task.Used_CPU_Time);
         CPU_E := Next (CPU_E);
      end loop;
      ...
      if not After_Timed_Event_Expiration then
         TE_T.Update_Timer_Due_To_New_Running_Task (New_Task, CS_Time);
      end if;
   ...
   Heir_Task : Task_Id;    --  Local to 'Do_Scheduling'
   End_Of_Interrupt_Handler : Boolean;  --  Local to 'Do_Scheduling'
   ...
   begin
      ...
      pragma Debug (CDBG.Do_Scheduling0 (Interrupt_Nesting_Count,
                    Running_Task, Ready_Queue)); -- {CDBG}

      --  Called from an interrupt handler?

      if Interrupt_Nesting_Count = 0 then
         --  called after an interrupt handler

         End_Of_Interrupt_Handler := True;

         --  set Interrupt_Nesting_Count to its default value

         Interrupt_Nesting_Count := Outside_Of_Interrupts;

      elsif Interrupt_Nesting_Count = Outside_Of_Interrupts then
         --  called from outside an interrupt handler

         End_Of_Interrupt_Handler := False;

      else
         --  called form inside an interrupt handler
...
--        MaRTE.Direct_IO.Put (" DS_No_int ");
      ...
      --  CPU-time accounting and timer reprogramming

      if not In_Interrupt_Handler then
         pragma Debug (DBG.Assert (not After_Timed_Event_Expiration));

         --  Only something to be done if a context switch is needed and
         --  CPU-time accounting is enabled

         if Running_Task /= Heir_Task
           and then (CP.Use_CPU_Time_Clocks_And_Timers      or else
                       CP.Use_Round_Robin_Scheduling_Policy or else
                         CP.Use_Sporadic_Server_Scheduling_Policy)
         then
            --  Perform here the job done by Start/End_Interrupt when this
            --  procedure is called from an interrupt handler

            CS_Time := HAL.Get_HWTime; --  get context switch time

            CPU_Time_Accounting (Old_Task => Running_Task,
                                 New_Task => Heir_Task,
                                 Now      => CS_Time);
         end if;

      else  --  In interrupt handler
      ...
      if Running_Task /= Heir_Task then
         ...
         pragma Debug -- {CDBG}
           (CDBG.Do_Scheduling2 -- {CDBG}
            (Running_Task, Heir_Task, Ready_Queue, CS_Time)); -- {CDBG}

         if not HAL.Context_Switch_In_Interrupt_Handler'First then
            --  Kernel nesting level
            Running_Task.Kernel_Nesting_Level := K.Get_Kernel_Nesting_Level;
            K.Set_Kernel_Nesting_Level (Heir_Task.Kernel_Nesting_Level);
         end if;

         --  The context switch is actually carried out in
         --  'Change_To_Context' or 'Context_Switch'.

         Former_Task := Running_Task;
         pragma Compile_Time_Warning
           (True, "no actualizar former_Task si Is_CS_Pending?");
         Running_Task := Heir_Task;
         if Former_Task.Magic = TERMINATED then
            if HAL.Context_Switch_In_Interrupt_Handler'First then
               HAL.Context_Switch
                 (Old_Task => Former_Task.Arch_Specific_Context'Address,
                  New_Task => Heir_Task.Arch_Specific_Context'Address);
            else
            ...
            Pool_TCBs.Release_TCB (Former_Task);
            ...
            if HAL.Context_Switch_In_Interrupt_Handler'First then
               ...
               HAL.Context_Switch
                 (Old_Task => Former_Task.Stack_Ptr'Address,
   ...
   ---------------------------
   -- Context_Switch_Window --
   ---------------------------

   procedure Context_Switch_Window is
   begin
      if HAL.Context_Switch_In_Interrupt_Handler'First then
         pragma Assert (K.Get_Kernel_Nesting_Level = 1);
      
         K.Set_Kernel_Nesting_Level (0);
         HAL.Enable_Interrupts;
         --  To allow the software interrupt handler to perfom the
         --  context switch.
         HAL.Disable_Interrupts;
         K.Set_Kernel_Nesting_Level (1);
         
      else
         null;
      end if;
   end Context_Switch_Window;

--More_Than_One_Int_Delete : Boolean := False;
--pragma Volatile (More_Than_One_Int_Delete);

   ---------------------
   -- Start_Interrupt --
   ---------------------

   procedure Start_Interrupt is
   begin
--        MaRTE.Direct_IO.Put (" SI ");

      pragma Debug (CDBG.Start_Interrupt (Interrupt_Nesting_Count,
        Running_Task)); -- {CDBG}

      --  Flag "in interrupt" shouldn't be set

      pragma Debug(DBG.Assert (not In_Interrupt_Handler));

      --  Set "in interrupt" flag

      In_Interrupt_Handler := True;

      if CP.Use_CPU_Time_Clocks_And_Timers       or else
        CP.Use_Round_Robin_Scheduling_Policy     or else
        CP.Use_Sporadic_Server_Scheduling_Policy or else
        CP.Measure_Interrupt_Handlers_Time'First then
         --  read clock (interrupt starting time)

         CS_Time := HAL.Get_HWTime;
      end if;

      Heir_Task := Running_Task;
      --  necessary for CPU_Time_Accounting when Do_Scheduling is not called
   ...
   procedure End_Interrupt is
   begin
--        MaRTE.Direct_IO.Put (" EI ");

      pragma Debug (CDBG.End_Interrupt (Interrupt_Nesting_Count,
        Running_Task)); -- {CDBG}

      pragma Debug(DBG.Assert (In_Interrupt_Handler));
      --  Flag "in interrupt" should be set

      if (CP.Use_CPU_Time_Clocks_And_Timers      or else
            CP.Use_Round_Robin_Scheduling_Policy    or else
              CP.Use_Sporadic_Server_Scheduling_Policy)
          and then
            (CP.Account_Interrupt_Handlers_Time_Separately'First or else
                   Running_Task /= Heir_Task)
      then
         --  Even if running and heir tasks are the same, CPU time accounting
         --  must be performed in other to avoid effect of interrupt handlers
         --  in CPU clocks when desired

         CPU_Time_Accounting (Old_Task => Running_Task,
                              New_Task => Heir_Task,
                              Now      => CS_Time);

      elsif CP.Measure_Interrupt_Handlers_Time'First then
         --  Account interrupt time

         TMRI.Account_Interrupt_Handler_Execution_Time
           (CS_Time - HAL.Get_HWTime);
      end if;

      --  Reprogram hardware timer when neccessary
   ...
   procedure Before_ATC is
   begin
      if In_Interrupt_Handler then
         pragma Assert (not HAL.Are_Interrupts_Enabled);

         --  It comes from the timer interrupt handler, so here it executes
         --  the final instrucctions of that procedure
         ...
         --  Re-enable timer interrupt in the interrupt controller

         MaRTE.HAL.End_Of_Timer_Interrupt;

         --  Now the final instructions of the MaRTE interrupt handler wrapper
         --  (Linux: marte_signal_handler_wrapper (interrupts.c), x86:
         --  base_irq_inittab.S)
   ...
   begin
      while True loop
         --  pragma Debug (DBG.Assert (HAL.Are_Interrupts_Enabled));
         null;
      end loop;
