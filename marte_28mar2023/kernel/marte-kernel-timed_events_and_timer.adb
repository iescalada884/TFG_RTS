...
package body MaRTE.Kernel.Timed_Events_And_Timer is
   ...
   begin
      ...
      pragma Debug (Debug.Reprogram_Timer_After_Expiration_0);
      ...
      if TEQ.Is_Empty then
         if Is_Empty (Running_Task.CPU_Time_TEs_Q) then
            ...
            if not Compulsory_Timer_Reprogramming then
               ...
               pragma Debug (Debug.Reprogram_Timer_After_Expiration_Do_Not);
               return; -- Do not reprogram the timer
            end if;
         else
            --  'TEQ' empty and 'CPU_Time_TEs_Q' NOT empty     
            pragma Debug (Debug.Reprogram_Timer_After_Expiration_CPU);
         ...
         pragma Debug (Debug.Reprogram_Timer_After_Expiration_EQHead);
   ...
   begin
      ...
      if (not Is_Empty (Running_Task.CPU_Time_TEs_Q) and then
         ...
         pragma Debug (Debug.More_Urgent_CPU_Event (Time_To_Program));
   ...
   begin
      ...
      pragma Debug (Debug.Extract_Expired_Event (Now,
                    Suspension_Time_Minimum,
                    TEQ.Head,
                    Timed_Event_Ac (Head (Running_Task.CPU_Time_TEs_Q))));

      if TEQ.Is_Empty then
         if not Is_Empty (Running_Task.CPU_Time_TEs_Q) then
            --  'TEQ' empty and 'CPU_Time_TEs_Q' NOT empty
            Ret := Timed_Event_Ac (Head (Running_Task.CPU_Time_TEs_Q));
            if Ret.T < Now + Suspension_Time_Minimum then      
               pragma Debug (Debug.Extract_CPU_Event (Ret.T));
         ...
         --  'TEQ' NOT empty and 'CPU_Time_TEs_Q' empty
         if TEQ.Head.T < Now + Suspension_Time_Minimum then
      ...
      pragma Debug (Debug.No_Expired_Events);
      return null;
...
--        Alarm_Time : HWI.HWTime;
--        pragma ImPort (C, Alarm_Time, "__gnat_alarm_time");
--        --  Defined in s-bbbosu.adb
   begin
--        return Alarm_Time;
