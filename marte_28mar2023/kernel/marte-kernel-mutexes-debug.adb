...
package body MaRTE.Kernel.Mutexes.Debug is
   ...
   begin
      if Mutexes_Debug_Messages then
         ...
         if Mutex (M.all).Owner = null then
            ...
            Put (",("); Put (Mutex (M.all).Owner.Id); Put (")");
         end if;
         Put ("}");
      end if;
   ...
   procedure Show_Mutex (M : Mutex_Descriptor) is
   begin
      if Mutexes_Debug_Messages then
         Put ("{"); Put (M.Id);
         Put (","); Put (Integer (M.Prio));
         Put (","); Put (Integer (M.Preemption_Level));
         if M.Owner = null then
            Put (",Free");
         else
            Put (",O:("); Put (Integer (M.Owner.Id));
         end if;
         Put ("),BT:"); Tasks_Lists_Show.Show_On_Console (M.Blocked_Tasks);
         Put ("}");
      end if;
   end Show_Mutex;
   ...
   procedure Show_Task (T : Task_Id) is
   begin
      if Mutexes_Debug_Messages then
         Put ("("); Put (T.Id);
         Put (","); Put (Integer (T.Active_Prio));
         Put (","); Put (Integer (T.Active_Preemption_Level));
         Put (",MO:");
   ...
   --  Called from CVI.Timeout_Reached

   procedure Task_Gets_Ready_Owning_Mutex (T : in Task_Id;
                                           M : in Mutex_Descriptor) is
   begin
      MaRTE.Spy.Send_Event (MaRTE.HAL.Get_HWTime, "MI",
                            M.Id, M.Prio);
      if Mutexes_Debug_Messages then
         Put (" | mg");
         Show_Task (T);
