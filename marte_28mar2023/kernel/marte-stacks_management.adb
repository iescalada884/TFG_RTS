...
with MaRTE.Stacks_Management.Debug;
...
package body MaRTE.Stacks_Management is
   ...
   package STACK_DBG renames MaRTE.Stacks_Management.Debug;
   ...
   begin
      pragma Debug
        (STACK_DBG.Initialize_Stack (Id_Count, Stack_Top, Size_In_DWords));
      ...
      --  Mark the stack borders with a magic value to detect overflows

      if S /= Main_Task_Stack or else HAL.Main_Task_Stack_Limits_Available then
         --  For tasks different form the main, the stack limits are always
         --  known, for the main task they are known in some architectures
   ...
   begin
      pragma Debug (DBG.Assert (HAL.Main_Task_Stack_Limits_Available
   ...
   function Stack_OK (Task_Stack_Top : in Unsigned_32;
      ...
      Is_OK : Boolean := False;
   begin
      if S = Main_Task_Stack'Access
        and then not HAL.Main_Task_Stack_Limits_Available then
         --  In architectures where the main stack limits aren't known, the
         --  only check we can do with the main task stack is if it is in use
         --  or not
      ...
      Is_OK :=
        S.Magic = STACK_USED                                     and then
        S.Stack (Stack_Memory_Area'First)     = MAGIC_LABEL_EOS  and then
        S.Stack (Stack_Memory_Area'First + 1) = MAGIC_LABEL_EOS  and then
        Task_Stack_Top >= Get_Stack_Top_Address (S)              and then
        Task_Stack_Top <= Get_Stack_Base_Address (S)             and then
        Unsigned_32 (S.Size_In_DWords - 1) * 4 =
        Get_Stack_Base_Address (S) - Get_Stack_Top_Address (S);

      pragma Debug (STACK_DBG.Show_Stack (S, Is_OK,
                    Task_Stack_Top, Use_Task_Stack_Top => True));
   ...
   procedure Configure_Main_Task_Stack is
      ...
      Top  : Stack_Pos;
   begin
      if HAL.Main_Task_Stack_Limits_Available then
         Base := UC (HAL.Main_Task_Stack_Base);
         Top  := UC (HAL.Main_Task_Stack_Top_Limit);
         pragma Debug (DBG.Assert (Base > Top));
         pragma Debug (DBG.Assert (Base - Top = CP.Main_Stack_Size_In_Bytes));
