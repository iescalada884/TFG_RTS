...
package MaRTE.Kernel is
   ...
   type TCB is tagged record
      ...
      --  task isn't running.
      --  In ARCH_GNAT_BB_ARM this field is not used and Arch_Specific_Context
      --  is used instead.
      ...
      --  Relative deadline of the task. Used for EDF tasks. Only used to store
      --  the relative deadline provided at task creation as attribute to be
      --  able to recover it when necessary (TOI.Get_Self_Attributes). It is
      --  not used for scheduling purposes, Task_Urgency and Deadline are used
      --  instead.
      ...
      Arch_Specific_Context : aliased HAL.Arch_Specific_Context_T;
      --  Only used in ARCH_GNAT_BB_ARM and ARCH_STM32 to store the registers
      --  of a preemted task. Plays de role of field
      --  System.BB.Threads.Thread_Descriptor.Context in the bare board RTSs.
   ...
   function Get_Kernel_Nesting_Level return Natural;

   procedure Set_Kernel_Nesting_Level (New_Value : Natural);
