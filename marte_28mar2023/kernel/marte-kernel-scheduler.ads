...
package MaRTE.Kernel.Scheduler is
   ...
   procedure Context_Switch_Window;

   ---------------------
   -- Start_Interrupt --
   ---------------------
   --  Called at the beginning of the asembler interrupt handler
   --  (base_irq_inittab.S)

   procedure Start_Interrupt;
   pragma Export (C, Start_Interrupt, "scheduler__start_interrupt");
   pragma Inline (Start_Interrupt);
   ...
   pragma Export (C, End_Interrupt, "scheduler__end_interrupt");
   pragma Inline (End_Interrupt);

   ----------------
   -- Before_ATC --
   ----------------

   procedure Before_ATC;
   pragma Export (C, Before_ATC, "scheduler__before_atc");
