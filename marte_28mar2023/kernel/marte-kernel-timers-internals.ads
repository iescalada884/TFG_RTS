...
package MaRTE.Kernel.Timers.Internals is
   ...
   procedure Account_Interrupt_Handler_Execution_Time
     (Interval : HAL.HWTime);
   --  Adds the interval to the time consumed by interrupt handlers. This time
   --  can be read by the application using the CLOCK_INTERRUPT_CPUTIME POSIX
   --  clock.

   function Get_Interrupt_Handlers_Execution_Time
     return HAL.HWTime;
   --  Returns the time consumed by the interrupt handlers.
