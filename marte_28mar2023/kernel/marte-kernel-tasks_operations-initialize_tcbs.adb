...
package body MaRTE.Kernel.Tasks_Operations.Initialize_TCBs is
   ...
   package TS           renames MaRTE.Timespec;
   ...
   procedure Initialize
     (T                 : in Task_Id;
      ...
      use K.Tasks_Lists;
      use type K.Timed_Event_Ac, HAL.HWTime;
      use type MaRTE.Stacks_Management.Stack_Id;
      Stack_Offset : Positive;
      Second_Value_In_Stack : MaRTE.Integer_Types.Unsigned_32;
   begin
      T.Magic                   := K.ACTIVE;
      T.Id                      := Id;
      --  T.Task_Type is set in 'Pool_TCBs.Request_TCB'
      T.Status                  := BLOCKED;
      if Id = Pool_TCBs.Main_Task_Id then
         T.Kernel_Nesting_Level := 0;

      else
         T.Kernel_Nesting_Level := 1;
         --  any task but the main calls "leave_kernel" at its
         --  first activation just before start running its body
      end if;
      ...
      T.Deadline := HAL.Get_HWTime;
      if Attr.Rel_Deadline > HAL.HWTime'Last - T.Deadline then
         --  If Attr.Rel_Deadline is HAL.HWTime'Last (default value) or it's
         --  too long that Attr.Rel_Deadline + HAL.Get_HWTime > HAL.HWTime'Last
         --  what would produce an exception if we make the addition
         T.Deadline := HAL.HWTime'Last;
      else
         --  A deadline lower than HAL.HWTime'Last can be set
         T.Deadline := T.Deadline + Attr.Rel_Deadline;
      end if;
      ...
      case Attr.Sched_Policy is
         ...
         when SCHED_RR =>
            ...
            if CP.Use_Round_Robin_Scheduling_Policy then
               T.Capacity := TS.Timespec_To_HWTime
                 (TS.NS_To_Timespec (CP.Round_Robin_Interval_Time_Period_NS));
            end if;
      ...
      if T.Task_Type /= K.MAIN_TASK then
         ...
         --  XXX stack initialization should be performed by a procedure in
         --  MaRTE.HAL in order to avoid this dependency with the architecture.
         case CP.MaRTE_Architecture'First is
            when CP.ARCH_RPI =>
               --  It is different from the X86 initialization since we must
               --  leave room for 12 registers
               Stack_Offset := 13;

               --  In ARM the second value in the stack is the initial value
               --  of the CPRS:
               --          31   27   23   19   15   11   7    4
               --          |    |  J |    _GE_ |    | EA IFT_mode_
               --          0000 0000 0000 0000 0000 0011 0001 1111
               --          0000 0000 0000 0000 0000 0011 0001 0011 [OLD]
               -- ARM mode, IRQ y FIQ enabled and mode SVC (supervisor) [OLD]
               -- Second_Value_In_Stack := 16#313#; [OLD]
               -- ARM mode, IRQ y FIQ enabled and mode "System"
               Second_Value_In_Stack := 16#31F#;

            when CP.ARCH_GNAT_BB_ARM | CP.ARCH_STM32F | CP.ARCH_EV3 =>
               pragma Compile_Time_Warning (True, "HAL.Init_Task_Context for EV3");
               ...
               return;  --  Nothing more to do for ARCH_GNAT_BB_ARM

            when CP.ARCH_X86 | CP.ARCH_LINUX |
                 CP.ARCH_LINUX_LIB | CP.ARCH_XTRATUM =>
               Stack_Offset := 1;
               Second_Value_In_Stack := 0;
         end case;

         --  Configure the task stack for the first activation (x86)
         --
         --  Stack_Ptr -> --  --  --  --  --  --  --  --  --  --  --  --  --
         ...
         MaRTE.Stacks_Management.Write_In_Stack (Addr => Task_Wrapper,
                                           P    => Stack_Offset + 2,
                                           S    => T.Stack);
         MaRTE.Stacks_Management.Write_In_Stack (Int => Second_Value_In_Stack,
                                           P   => Stack_Offset + 1,
                                           S   => T.Stack);
         MaRTE.Stacks_Management.Write_In_Stack
           (Addr => Task_Body_Function_To_Address (Task_Body),
            ...
            S    => T.Stack);
         T.Stack_Ptr :=
           MaRTE.Stacks_Management.DWord_In_Stack_Address
             (P => Stack_Offset + 2, S => T.Stack);
      end if;
   end Initialize;
