...
package MaRTE.Spy is
   ...
   procedure Send_Event_If_Initialized (Operation : in String;
                                        Param1    : in Integer;
                                        Param2    : in Integer);
   --  To be used in kernel procedures that could be called at the very
   --  beginning of program execution when traces are not initialized yet,
   --  but they will be called also latter when everything is properly setup.

   pragma Inline (Send_Event_If_Initialized);
end MaRTE.Spy;
