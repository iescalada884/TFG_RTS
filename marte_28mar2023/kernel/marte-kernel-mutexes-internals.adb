...
package body MaRTE.Kernel.Mutexes.Internals is
   ...
   begin
      ...
      --  As an special case, the idle task can lock a mutex from an Ada timing
      --  event handler that has expired while this task was executing. Since
      --  the handler is executed in interrupt context (what, in fact, is a
      --  non-preemptive region) no locking actions are required

      if SCHD.Self.Task_Type = IDLE_TASK then
         return;
      end if;
