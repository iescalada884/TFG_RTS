...
package body MaRTE.Kernel.Pool_TCBs is
   ...
   procedure Release_TCB (T : in Task_Id) is
      use type K.Task_Types, Interfaces.C.int;
   begin
      ...
      if T.Detach_State = PTHREAD_CREATE_JOINABLE then
         return;
         --  Nothing to do right now. Status will be changed to detached in
         --  TO.Pthread_Join and this procedure will be called again.
      end if;
