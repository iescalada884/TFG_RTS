...
package body MaRTE.Kernel.Tasks_Operations is
      ...
      case Policy is
         ...
         when SCHED_RR =>
            if CP.Use_Round_Robin_Scheduling_Policy then
               T.Capacity := Timespec.Timespec_To_HWTime
                 (Timespec.NS_To_Timespec
                    (CP.Round_Robin_Interval_Time_Period_NS));
   ...
   function Pthread_Setdeadline (T         : K.Task_Id;
      ...
      Flags : Integer;
   begin
      pragma Debug (Debug.Setdeadline (T, Deadline, Immediate));

      if not CP.Use_EDF_Scheduling_Policy'First then
         return OPERATION_NOT_IMPLEMENTED;
      end if;

      --  Only Realtime and monotonic clocks are permited

      if Clock /= K.Timers.CLOCK_REALTIME and then
        Clock /= K.Timers.CLOCK_MONOTONIC
      then
         return OPERATION_NOT_PERMITTED;
      end if;

      --  A deadline based on clock realtime cannot specify a time before the
      --  initial offset because it cannot be represented with a valid value of
      --  monotonic time

      if Clock = K.Timers.CLOCK_REALTIME and then
        TSPC.Timespec_To_HWTime (Deadline.all) <
        MaRTE.Kernel.Timers.Realtime_Clock_Offset
      then
         return INVALID_ARGUMENT;
      end if;
      ...
      --  Set the deadline field in the TCB.
      --  If it is based on the CLOCK_REALTIME it has to be modified to be
      --  based on the monotonic clock.

      T.Deadline := TSPC.Timespec_To_HWTime (Deadline.all);
      if Clock = K.Timers.CLOCK_REALTIME then
         T.Deadline := T.Deadline - MaRTE.Kernel.Timers.Realtime_Clock_Offset;
      end if;

      --  Actions based on task's policy
      ...
      if T.Sched_Policy = SCHED_EDF then
         ...
         if Immediate = 1 then
            ...
            T.Base_Urgency := Task_Urgency (HAL.HWTime'Last - T.Deadline);
         ...
         --  If not immediate just set deadline field in the TCB (done
         --  previously). It will be translated to a urgency value the next
         --  time the task is reactivated after having been suspended or
         --  blocked

      else  --  Not SCHED_EDF
         --  Just set deadline field in the TCB (done previously). It will be
         --  translated to a urgency value when the policy of the task change
         --  to SCHED_APP
   ...
   begin
      ...
      if Clock = K.Timers.CLOCK_MONOTONIC then
         Deadline.all :=
           TSPC.HWTime_To_Timespec (HAL.HWTime'Last
                                    - HAL.HWTime (T.Base_Urgency));
      else
         pragma Debug (DBG.Assert (Clock = K.Timers.CLOCK_REALTIME));

         --  If time is based on CLOCK_REALTIME the Realtime_Clock_Offset has
         --  to be added (unless this addition would produce an overflow that
         --  HAL.HWTime'Last is returned instead)

         if HAL.HWTime (T.Base_Urgency)
           > MaRTE.Kernel.Timers.Realtime_Clock_Offset
         then  --  No overflow
            Deadline.all := TSPC.HWTime_To_Timespec
              (HAL.HWTime'Last
               - HAL.HWTime (T.Base_Urgency)
               + MaRTE.Kernel.Timers.Realtime_Clock_Offset);

         else  -- Overflow avoided
