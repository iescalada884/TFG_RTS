 ...
 *
 *
 * File 'pcnet32.c'                                         Modified By Chema.
 *                                                          Jose Maria Martinez
 *                                                          <chema@gmx.net>
 *
 * The hardware driver for the ethernet card AMD Lance PCI.
 *
 * {CHEMA_COPYRIGHT}
 *
 *---------------------------------------------------------------------------*/
...
static struct pci_id pcnet32_nics[] = {
  ...
  {0x1022, 0x2625, "AMD Lance/PCI PCNet/32"},
  {0x1022, 0x2001, "AMD Lance/HomePNA"},
};
...
static int pcnet32_init_ring(struct nic *nic)
{
  ...
  lp->init_block.rx_ring = (u32) (&rx_ring[0]);
  lp->init_block.tx_ring = (u32) (&tx_ring[0]);
  return 0;
}
...
static void pcnet32_reset(struct nic *nic)
{
  ...
  lp->init_block.mode =
    ((lp->options & PCNET32_PORT_PORTSEL) << 7);
  ...
  lp->a.write_csr(ioaddr, 1,
		  ((unsigned long) (&lp->init_block)) & 0xffff);
  lp->a.write_csr(ioaddr, 2, ((unsigned long)(&lp->init_block)) >> 16);
...
***************************************************************************/
static int pcnet32_poll(struct nic *nic )
{
  ...
  status = ((short) (rx_ring[entry].status) >> 8);
  ...
  if (status == 0x03) {
    //printf("Eth_Received: Index: %d \n",entry);
    nic->packetlen =
      ((rx_ring[entry].msg_length) & 0xfff) - 4;
    memcpy(nic->packet, &rxb[entry], nic->packetlen);

    /* Andrew Boyd of QNX reports that some revs of the 79C765
     * clear the buffer length */
    rx_ring[entry].buf_length = (-PKT_BUF_SZ);
    rx_ring[entry].status |= (0x8000);	/* prime for next receive */
...
{				/* Packet */
  ...
  tx_ring[entry].length = -s;
  tx_ring[entry].misc = 0x00000000;
  tx_ring[entry].base = (u32) (ptxb);
  ...
  while ((short) (tx_ring[entry].status) < 0);

  if ((short) (tx_ring[entry].status) < 0)
...
/* Function: pcnet32_handler
 *
 * Description: Checks if we have received an interrupt triggered packet
 *
 * Arguments: void
 *
 * Returns:   int.(1 if there is a pending frame and 0 if not)
 */


static int pcnet32_handler(unsigned short protocol_filter){

  u16 status;
  u16 protocol;
  int entry; //,i;

  /* We check for a proper rx frame. */
  // static int counter = 0;
  status = lp->a.read_csr(ioaddr, 0);

  if (status & 0x0400) { // Rx Interrupt
    /* Acknowledge all of the current interrupt sources ASAP. */
    //  lp->a.write_csr (ioaddr, 0, status & ~0x004f);
    lp->a.write_csr (ioaddr, 0, status & 0x0400);

    /* Set interrupt enable. */
    lp->a.write_csr (ioaddr, 0, 0x43);
    ...
    entry = lp->cur_rx & RX_RING_MOD_MASK;
    protocol = ntohs( *(u16 *)(&rxb[entry] + 2 * ETH_ALEN) );
    //protocol = ntohs( (u16) (rxb[entry + 2 * ETH_ALEN]) );
    //    printf("Frame(%d): Status: %X - Protocol is: %X; Index: %d\n",
    //	   ++counter, status, protocol, entry);
    /* We check if it is a valid protocol Frame */
    if( protocol == protocol_filter) {
      //printf("Protocol is RT-EP: %xd", protocol);
      return 1;
    } else {
/*       printf("==========>Discarting Frame:\n "); */
/*       for(i=0; i< 16 ; i++ ){  */
/*  	if ((i % 5) == 0) printf("\n");  */
/*  	printf(" (%d)%X -", i, (unsigned char) rxb[entry + i]);  */
/*       }  */
/*       printf("\n");  */
...
static int pcnet32_open(struct nic *nic){

  /* initialize device */
  pcnet32_reset(nic);
  /* Enable Rx Interrupps */
  lp->a.write_csr (ioaddr, 3, 0x00005B58);
  /* Trigger an initialization just for the interrupt. */
  lp->a.write_csr (ioaddr, 0, 0x43);
  /* Enable Promiscous mode, */
  lp->a.write_csr (ioaddr, 15, 0x08000);
...
***************************************************************************/
int pcnet32_probe(struct dev *dev)
{
  ...
  for (i=0; i< sizeof(pcnet32_nics) / sizeof(pcnet32_nics[0]); i++){
    ret=pci_find_device(pcnet32_nics[i].vendor,
			pcnet32_nics[i].dev_id, NULL, &pci_dev);
    if (ret==0){
      pci_dev.name=pcnet32_nics[i].name;
      break;
    }
  } // end for
  ...
  a->write_csr(ioaddr, 1,
	       (u32)(&lp->init_block) & 0xffff);
  a->write_csr(ioaddr, 2,
	       (u32)(&lp->init_block) >> 16);
  ...
  nic->irq      = pci->irq; // pcnet32_irq;
  nic->handler  = pcnet32_handler;
  nic->open     = pcnet32_open;
