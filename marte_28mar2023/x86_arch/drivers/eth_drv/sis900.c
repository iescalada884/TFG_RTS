 ...
 *
 *
 * File 'sis900.c'                                           Modified by Chema.
 *                                                          Jose Maria Martinez
 *                                                            <chema@gmx.net>
 * The hardware driver for the ethernet card SIS 900.
 *---------------------------------------------------------------------------*/
/*****************************************************************************/
/* Most of this module is taken from the Etherboot                           */
/* proyect: http://etherboot.sourceforge.net v.5.1.3:                        */
...
//#define outl(val, port) outl_p(val, port)
...
int sis900_probe(struct dev *dev);
...
static int  sis900_open(struct nic *nic);
...
static void sis900_transmit(struct nic *nic, const unsigned char *d,
...
static int sis900_handler(unsigned short protocol_filter);
...
static int sis630e_get_mac_addr(struct pci_device * pci_dev, struct nic *nic)
{
	...
	memset(p, 0, sizeof(p));
	//	do {
	pci_find_device(0x1039, 0x0008, NULL, p);
	//} while(p->driver && p->driver != &sis_bridge_driver);
	for (i=0;
	     i < sizeof(pci_isa_bridge_list)/sizeof(pci_isa_bridge_list[0]);
	     i++){
	  ret=pci_find_device(pci_isa_bridge_list[i].vendor,
			      pci_isa_bridge_list[i].dev_id, NULL, p);
	  if (ret==0)
	    break;
	}
	/* error on failure */
...
/* Function: sis900_create
 *
 * Description: Creates and find the eth interface.
 *
 *
 *
 *
...
int sis900_probe(struct dev *dev){
  ...
  for (i=0;i< sizeof(sis900_nics)/sizeof(sis900_nics[0]); i++){
    ret=pci_find_device(sis900_nics[i].vendor,
			sis900_nics[i].dev_id, NULL, &pci_dev);
    if (ret==0){
      pci_dev.name=sis900_nics[i].name;
      break;
    }
  }
  ...
  /* There are enough "SIS900" strings on the console already, so
  ...
  nic->handler  = sis900_handler;
  nic->open     = sis900_open;
  nic->irq      = pci->irq;
...
static int sis900_handler(unsigned short protocol_filter){
  ...
  if (status & RxOK){
    ...
    protocol = ntohs( *(u16 *) (rxb + cur_rx*RX_BUF_SIZE + 2 * ETH_ALEN) );
...
static int sis900_open(struct nic *nic){
  // extracted from probe

    /* initialize device */
    sis900_init(nic);

    /*We enable Rx OK interrupt. */
    outl(RxOK, ioaddr + imr);
 ...
 *
 * Description: enable interrupts and soft resets the controller chip
 *
 * Arguments: struct nic *nic:          NIC data structure
 *
 * Returns:   void.
...
sis900_reset(struct nic *nic)
{
    ...
    //u8 revision;
    /*We enable interrupts*/
    outl(0x00000001, ioaddr + ier);
    /*We enable Rx OK interrupt. */
    outl(0x00000001, ioaddr + imr);
 ...
 *    // Receive all paquetes on the net.
 * Arguments: struct nic *nic:          NIC data structure
 *
 * Returns:   void.
 */
...
static void sis900_set_rx_mode(struct nic *nic)
{
    ...
    /* Accept All physical packets. */
    outl(RFEN | RFPromiscuous, ioaddr + rfcr);
...
sis900_read_mode(struct nic *nic, int phy_addr, int *speed, int *duplex)
{
	...
	*duplex = FDX_CAPABLE_FULL_SELECTED;
    ...
    if (status & MII_STSOUT_LINK_FAIL)
        printc("sis900_read_mode: Media Link Off\n");
    else
        printc("sis900_read_mode: Media Link On %s %s-duplex \n",
               *speed == HW_SPEED_100_MBPS ?
               "100mbps" : "10mbps",
               *duplex == FDX_CAPABLE_FULL_SELECTED ?
               "full" : "half");
...
/*     nstype = htons(t); */
    ...
    // In RT with RT-EP this isn't needed.
    /*
    to = currticks() + tx_timeout;

    while ((((volatile u32) tx_status=txd.cmdsts) & OWN) &&
	   (currticks() < to)){
      // wait
    }
     if (currticks() >= to) {
