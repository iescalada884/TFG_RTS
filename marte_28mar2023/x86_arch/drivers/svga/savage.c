...
extern void Trio64_qualify_cardspecs(CardSpecs * cardspecs, int dacspeed);
extern void Trio64_init(void);
...
enum { UNKNOWN, TRIO_64, TRIO3D, TRIO3D2X,VIRGE, VIRGEVX, VIRGEDX, VIRGEGX2, VIRGEMX,SAVAGE3D, SAVAGEMX, SAVAGE4, SAVAGEPRO, SAVAGE2000 };

/*MaRTE OS*/
static unsigned long buf[64];/*Devuelto por el PCI*/
int CHIPSET;/*Identificador PCI de la tarjeta*/
...
#undef NL
    ...
    /*MaRTE OS*/
    savage_saveregs(moderegs);
    
    new = (vgaS3VPtr)(moderegs+VGA_TOTAL_REGS);
   
    /*MaRTE OS*/
    /*    if(modeinfo->bitsPerPixel==16) {
        if((chipset==VIRGE)|| (chipset==TRIO_64)) {
...
int savage_test(void)
{
  ...
  if(found){
    printf("Savage Card Not Found!\n");
    return 0;
  }
  CHIPSET=(buf[0]>>16)&0xffff;
  
  switch(CHIPSET) {
  case 0x8811:
  case 0x8901:/*MaRTE OS*/
    chipset = TRIO_64;
    break;
  case 0x8903:
  case 0x8904:
  ...
  default:
    chipset = UNKNOWN;
    printf("Savage Card Not Soported!\n");
    return 0;
  }
  savage_init(0,0,0);
  return 1;
...
static int savage_init(int force, int par1, int par2)
{
  ...
  cardspecs->flags = INTERLACE_DIVIDE_VERT;/*MaRTE OS*//* | CLOCK_PROGRAMMABLE;*/
  ...
  if (chipset==TRIO_64){
    ...
    Trio64_init();
    Trio64_qualify_cardspecs(cardspecs, 0);
/*MaRTE OS*/
#ifndef INCLUDE_S3_TRIO64_DAC
  }else{
  ...
  /*MaRTE OS*/
  /*No funciona en modo 16 colores*/
