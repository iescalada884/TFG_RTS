...
procedure test_rtep_distributed_mutex is
   ...
   procedure Other_Slave_Job is
      Other_Slave_Prio : RTEP.Priority := 4;
   begin
      Put_Line ("OTHER SLAVE");
      loop
         delay 0.5;
         Put ("LOCKING ...  ");
         RP.Lock_Distributed_Mutex (M, Other_Slave_Prio);
         Put_Line ("Mutex FOR ME!!!");
         delay 5.0;
         RP.Unlock_Distributed_Mutex (M, Other_Slave_Prio);
         Put_Line ("Mutex RELEASED!!!");
         delay 1.5;
      end loop;
   end Other_Slave_Job;
...
exception
   when E : others =>
      Put_Line ("ERROR test_rtep_distributed_mutex");
      Put_Line (Ada.Exceptions.Exception_Name (E));
      Put_Line (Ada.Exceptions.Exception_Message (E));
end test_rtep_distributed_mutex;
