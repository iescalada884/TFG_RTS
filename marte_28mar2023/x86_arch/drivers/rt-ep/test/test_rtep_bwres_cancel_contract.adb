...
procedure Test_RTEP_BWRES_Cancel_Contract is
   ...
   Budget_Min : RTEP.Network_Budget := 5; -- packets
   ...
   loop
      ...
      (Destination_Station_ID  => Dest_Id,
         ...
         Data => RTEP_Streams.To_Stream_Element_Array
            ("Hello World "&Integer'Image (I)),
         Id                      => S_Id,
         Blocking                => True);
      I := I + 1;

      --  delete contract
      Ada.Text_Io.Put_Line ("DELETING CONTRACT...");
      -- delay 10.0;
      Check_NZ (BW.Cancel_Contract (Vres_Id));
      Show_All;

      --  Negotiate the contract again
      Ada.Text_Io.Put_Line ("NEGOTIATING CONTRACT AGAIN...");
      Accepted := BW.Negotiate_Contract (Contract, Vres_Id);
      if Accepted /= 0 then
         Ada.Text_Io.Put_Line ("Contract Not Accepted");
         return;
...
exception
   when The_Error : others =>
      ...
      raise;
end Test_RTEP_BWRES_Cancel_Contract;
