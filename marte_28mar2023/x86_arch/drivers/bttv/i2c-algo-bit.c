...
/* $Id: i2c-algo-bit.c 53 2005-10-25 18:47:17Z cvsadm $ */
...
void add_nanosecs(struct timespec * time, int nanosecs)
{
  if (((unsigned long long)nanosecs+(unsigned long long)time->tv_nsec)>=1000000000){
    time->tv_sec=time->tv_sec+(time_t)1;
    time->tv_nsec=(int)(((unsigned long long)nanosecs+(unsigned long long)time->tv_nsec)-1000000000);
  }
  else
    time->tv_nsec=time->tv_nsec+nanosecs;
}
int is_greater(struct timespec time1,struct timespec time2)
{
...
static inline int sclhi(struct i2c_algo_bit_data *adap)
{
  //	unsigned long start;
	...
	while (! getscl(adap) ) {	
	  ...
	  if(is_greater(start2,start1)){
	    return -ETIMEDOUT;
	  }
		//cond_resched();
	}
	DEBSTAT(printk(KERN_DEBUG "needed (%d ,%d) -(%d ,%d)\n", start2.tv_sec,start2.tv_nsec,start1.tv_sec,start1.tv_nsec));
	udelay(adap->udelay);
	return 0;
} 
...
static int sendbytes(struct i2c_adapter *i2c_adap, struct i2c_msg *msg)
{
	...
	while (count > 0) {
		c = *temp;
		DEB2(dev_dbg3(&i2c_adap->dev, "sendbytes: writing %2.2X\n", c&0xff));
		...
		if ((retval>0) || (nak_ok && (retval==0)))  { /* ok or ignored NAK */
			...
			dev_err2(&i2c_adap->dev, "sendbytes: error - bailout.\n");
			i2c_stop(adap);
...
/*printk("%#02x ",inval); if ( ! (count % 16) ) printk("\n"); */
		if (inval>=0) {
			*temp = inval;
			...
			printke("i2c-algo-bit.o: readbytes: i2c_inb timed out.\n");
			break;
		}
		...
		if (sclhi(adap)<0) {	/* timeout */
			sdahi(adap);
			printke("i2c-algo-bit.o: readbytes: Timeout at ack\n");
			return -ETIMEDOUT;
		};
		scllo(adap);
		sdahi(adap);
...
static inline int bit_doAddress(struct i2c_adapter *i2c_adap, struct i2c_msg *msg) 
{
	...
	if ( (flags & I2C_M_TEN)  ) { 
		...
		if ((ret != 1) && !nak_ok)  {
			printke("died at extended address code.\n");
			return -EREMOTEIO;
		}
		...
		if ((ret != 1) && !nak_ok) {
			...
			printke("died at 2nd address code.\n");
			return -EREMOTEIO;
		}
		if ( flags & I2C_M_RD ) {
			i2c_repstart(adap);
...
int i2c_bit_add_bus(struct i2c_adapter *adap)
{
	...
	DEB2(dev_dbg2(&adap->dev, "hw routines registered.\n"));
