...
struct pci_device* bttv_get_pcidev(unsigned int card)
{
	if (card >= bttv_num)
		return NULL;
	return bttvs[card].bt_dev;
}
...
int init_bttv_i2c(struct bttv *btv)
{
	...
	i2c_fill_driver(&btv->i2c_client);
	if (use_hw) {
		/* bt878 */
	...
	btv->i2c_adap.dev.parent = NULL; //&btv->dev->dev;
