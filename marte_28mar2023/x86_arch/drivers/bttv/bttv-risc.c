		 ...
		 unsigned int padding, unsigned int lines)
{
	...
	instructions += 2;
	if ((rc = btcx_riscmem_alloc(btv->bt_dev,risc,instructions*8)) < 0)
		return rc;
		 ...
		 unsigned int cpadding)
{
	...
	instructions += 2;
	if ((rc = btcx_riscmem_alloc(btv->bt_dev,risc,instructions*4*5)) < 0)
		return rc;
...
int
bttv_risc_overlay_marte(struct bttv *btv, struct btcx_riscmem *risc,
		  const struct bttv_format *fmt, struct bttv_overlay *ov,
		  int skip_even, int skip_odd, int number_of_buffer)
{
	...
	*(rp++) = cpu_to_le32(0);
	...
	if(skip_odd==1 && btv->mode==NON_INTERLACED)
		addr  = (unsigned long)btv->mfbuf.base[number_of_buffer]+btv->mfbuf.fmt.bytesperline*ov->w.height/2;
	else
		addr  = (unsigned long)btv->mfbuf.base[number_of_buffer];
	addr += btv->mfbuf.fmt.bytesperline * ov->w.top;
	addr += (fmt->depth >> 3)          * ov->w.left;
	//	set_break_point_here;
	/* scan lines */
	for (maxy = -1, line = 0; line < ov->w.height;
	     line++, addr += btv->mfbuf.fmt.bytesperline) {
		if ((btv->opt_vcr_hack) &&
		     (line >= (ov->w.height - VCR_HACK_LINES)))
			continue;
 		if ((line%2) == 0  &&  skip_even){
			if(btv->mode==NON_INTERLACED)
				addr -= btv->mfbuf.fmt.bytesperline;	
			continue;
		}
...
bttv_risc_init_main(struct bttv *btv)
{
	...
	if ((rc = btcx_riscmem_alloc(btv->bt_dev,&btv->main,PAGE_SIZE)) < 0)
		return rc;
	dprintk(KERN_DEBUG "bttv%d: risc main %d @ %08Lx\n",
		btv->nr,btv->n_buffers,(unsigned long long)btv->main.dma);
...
bttv_dma_free(struct bttv *btv, struct bttv_buffer *buf)
{
	...
	btcx_riscmem_free(btv->bt_dev,&buf->bottom);
	btcx_riscmem_free(btv->bt_dev,&buf->top);
		  ...
		  struct bttv_buffer *buf)
{
	...
	/* calculate geometry */
	bttv_calc_geo(btv,&geo,ov->w.width,ov->w.height,
		      V4L2_FIELD_HAS_BOTH(ov->field), ov->tvnorm);
	for(i=0;i<btv->n_buffers;i++)
		memcpy(&btv->screen[i].geo,&geo,sizeof(struct bttv_geometry));
	//	set_break_point_here;
	/* build risc code */
	switch (ov->field) {
	case V4L2_FIELD_TOP:
		for(i=0;i<btv->n_buffers;i++)
			bttv_risc_overlay_marte(btv, &btv->screen[i].top,    fmt, ov, 0, 0, i);
		break;
	case V4L2_FIELD_BOTTOM:
		for(i=0;i<btv->n_buffers;i++)
			bttv_risc_overlay_marte(btv, &btv->screen[i].bottom, fmt, ov, 0, 0, i);
		break;
	case V4L2_FIELD_INTERLACED:
#if 0
		for(i=0;i<btv->n_buffers;i++){
			bttv_risc_overlay_marte(btv, &btv->screen[i].top,    fmt, ov, 1, 0, i);
			bttv_risc_overlay_marte(btv, &btv->screen[i].bottom, fmt, ov, 0, 1, i);
		}
#else
		for(i=0;i<btv->n_buffers;i++){
			bttv_risc_overlay_marte(btv, &btv->screen[i].top,    fmt, ov, 0, 1, i);
			bttv_risc_overlay_marte(btv, &btv->screen[i].bottom, fmt, ov, 1, 0, i);
		}
#endif
		break;
	default:
	...
	/* copy format info */
	for(i=0;i<btv->n_buffers;i++){
		btv->screen[i].btformat = fmt->btformat;
		btv->screen[i].btswap   = fmt->btswap;
		btv->screen[i].vb.field = ov->field;
	}
	return 0;
}
...
/* calculate geometry, build risc code */
int
bttv_overlay_risc_marte(struct bttv *btv,
		  struct bttv_overlay *ov,
		  const struct bttv_format *fmt)
{
	int i;
