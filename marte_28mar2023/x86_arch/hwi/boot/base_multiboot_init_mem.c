...
// #include <oskit/x86/pc/phys_lmm.h> MAR
...
void base_multiboot_init_mem(void)
{
	...
	do
	{
		...
		skip(kvtophys(_smarte), kvtophys(_emarte));
		...
		// phys_lmm_add(min, max - min); MAR

		/*
		 * Uncomment the following line to avoid the
		 * warm-reboot problem
		 */
		//memset(min, 0, max - min);
		...
		retry:
		; // <----  MaRTE OS (to keep gcc 3.4 happy)
	}
