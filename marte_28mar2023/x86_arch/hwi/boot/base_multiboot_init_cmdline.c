...
base_multiboot_init_cmdline(int *argcp, char ***argvp)
{
	...
	if (boot_info.flags & MULTIBOOT_CMDLINE) {
		...
		// < MAR
		// The Etherboot command line doesn't have a "--" so I supose
		// is the last argument
		// XXX with this I get the retaddress but it hungs the PC
		// when returning from _exit :(
		// if (dashdashi == 0)
		//   dashdashi = ntoks + 1;
		// MAR >
		...
		argbuf = lmm_alloc(&malloc_lmm,
				   ...
				   sizeof(char *) * (nbops + 1),
				   0); */
		argbuf = malloc(sizeof(char *) * (nargs + 1) +
				sizeof(char *) * (nvars + 1) +
				sizeof(char *) * (nbops + 1));
		/* MAR > */
		assert(argbuf);
