...
read(oskit_stream_t *si, void *buf, oskit_u32_t len, oskit_u32_t *out_actual)
{
	...
	int i;
	for(i=0; i < len; ++i) {
		((char*)buf)[i] = com_cons_getchar(s->port);
	}
	*out_actual = i;
	return 0;
}
      ...
      oskit_u32_t *out_actual)
{
	...
	int i=0;
	for(i=0; i < len; ++i) {
		com_cons_putchar(s->port, ((char*)buf)[i]);
	}
	return 0;
}
...
static struct strm com_console_impl;

oskit_error_t
com_console_init(int com_port, struct termios *com_params, 
		    oskit_stream_t **out_stream)
{
  com_console_impl.strmi.ops = &ops;
  com_console_impl.refs = 1;
  com_console_impl.port = com_port;

  com_cons_init(com_port, com_params);

  *out_stream = &com_console_impl.strmi;
  return 0;
}
