...
OSKIT_INLINE unsigned get_eflags()
{
	unsigned eflags;
	// << MaRTE OS  to avoid compilation warnings
	// asm volatile("
	// 	pushfl
	// 	popl %0" : "=r" (eflags));
	asm volatile("pushfl\n\t"
...
{
	unsigned eflags;
	// MaRTE OS  several strings to avoid compilation warnings
	asm volatile(
	"	jmp	1f\n\t"
	"1:	jmp	1f\n\t"
	"1:	jmp	1f\n\t"
...
OSKIT_INLINE void cli() { asm("cli"); }
OSKIT_INLINE void sti() { asm("sti"); }
OSKIT_INLINE void cld() { asm("cld"); }
#ifdef __cplusplus
/* in C++, std is a reserved keyword -- so we use a function macro instead */
#define std() asm("std")
#else
OSKIT_INLINE void std() { asm("std"); }
#endif
OSKIT_INLINE void clts() { asm("clts"); }

OSKIT_INLINE unsigned short get_cs()
{
	unsigned short cs;
...
OSKIT_INLINE unsigned short get_ds()
{
	unsigned short ds;
...
OSKIT_INLINE unsigned short get_es()
{
	unsigned short es;
...
OSKIT_INLINE unsigned short get_fs()
{
	unsigned short fs;
...
OSKIT_INLINE unsigned short get_gs()
{
	unsigned short gs;
...
OSKIT_INLINE unsigned short get_ss()
{
	unsigned short ss;
