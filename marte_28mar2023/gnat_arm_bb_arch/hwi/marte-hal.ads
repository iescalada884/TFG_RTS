...
package MaRTE.HAL is
   ...
   ARCH_GNAT_BB_ARM : constant CP.Supported_Architectures := 
     CP.ARCH_GNAT_BB_ARM;
   for ARCH_GNAT_BB_ARM'Size use MaRTE.Integer_Types.Int'Size;
   pragma Export (C, ARCH_GNAT_BB_ARM, "hal__gnat_bb_arm_arch");

   ----------------------------------------------------------------------------
   -- Interrupts --------------------------------------------------------------
   ----------------------------------------------------------------------------
   subtype HW_Interrupt is System.BB.Interrupts.Interrupt_ID;
   
   Sys_Tick_Vector : constant HW_Interrupt := 15;
...
--   pragma Convention (C, Default_HW_Interrupt_Handler); -- Necesario para ARM?
   ...
   subtype HWTime is System.BB.Time.Time;
   ...
   --  In this architecture the timer should always be reprogrammed even if
   --  there isn't any event (in order to keep the system clock). But in this
   --  case this reprogramming is carried out by the GNAT BB and MaRTE does
   ...
   --  In ARCH_GNAT_BB_ARM context switches are performed in the context of
   --  an interrupt handler.
   
   subtype Arch_Specific_Context_T is System.BB.CPU_Primitives.Context_Buffer; 
   
   procedure Context_Switch (Old_Task : in System.Address;
                             New_Task : in System.Address);
   pragma Inline (Context_Switch);
   --  For this architecture Old_Task and New_Task points to the
