...
package MaRTE_Semaphores is
   ...
   --  Abs_Timeout is a Duration instead of an absolute time
   --  (Ada.Real_Time.Time) because we want to avoid dependency with
   --  Ada.Real_Time in this package. (In GPL-2010 dependency with
   --  Ada.Real_Time implies dependency with System.Tasking!!, what
   --  didn't happen in GPL-2009)

   --  The user can do an unchecked conversion from Ada.Real_Time.Time
   --  to duration in order to use this function with an absolute time.
   function Timedwait (Sem         : in Semaphore_Ac;
                       Abs_Timeout : in Duration)
                      return Int;
