 ...
 *                                      C
 *
 *  File 'text_console.c'                        By MAR and
 *                                                  Miguel Ángel Masmano Tello.
 *
 *  MaRTE OS on Linux.
 *
 *  Console driver. Just a wrapper to 'linux_write' and 'linux_read'
 *  system calls.
 *
 *  ----------------------------------------------------------------------
...
extern int linux_read (int, void*, int);
extern int linux_write (int, void*, int);
...
{
  linux_read (0, (char *) buffer, bytes);
  return bytes;
}
...
{
  linux_write (1, (char *) buffer, bytes);
  return bytes;
}
