...
{
  int rv = 0; //::ioctl (m_fd, request, argument);
    if (rv < 0)
...
{
  int rv = 0; //::fcntl (m_fd, request, argument);
    if (rv < 0)
	set_and_throw (failbit, rname);
    return (rv);
}
...
{
  void* result = NULL;
  //::mmap (NULL, n, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd, offset);
   //  if (result == MAP_FAILED)
// 	set_and_throw (failbit, "mmap");
//     return (memlink (result, n));
  return (memlink((void*)0, n));
}
