...
--
--
--  File 'posix-files.adb'                               By Sangorrin
--
--
...
package body POSIX.Files is
   ...
   type char_ptr is access constant POSIX_Character;
   pragma Convention (C, char_ptr);

   function unlink (path : char_ptr) return MaRTE.Integer_Types.Int;
   pragma Import (C, unlink, "unlink");

   procedure Unlink (Pathname : in POSIX.Pathname) is
      Pathname_With_NUL : POSIX_String := Pathname & NUL;
   begin
      if (Pathname'Length + 1) > FSDT.Path'Length then
         PI.Raise_POSIX_Error (FILENAME_TOO_LONG);
      end if;

      PI.Check (unlink (Pathname_With_NUL
                 (Pathname_With_NUL'First)'Unchecked_Access));
   ...
   --         New_Pathname_With_NUL
   --  (New_Pathname_With_NUL'First)'Unchecked_Access));
   --    end Link;
   --
   --    --------------
   --    --  Rename  --
