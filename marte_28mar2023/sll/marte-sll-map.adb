...
package body MaRTE.SLL.Map is
   ...
   procedure Enqueue_In_Urgency_Order_Tail (E : in Element_Ac;
                                            L : in out List;
   ...
   end Enqueue_In_Urgency_Order_Tail;

   -----------------------------------
   -- Enqueue_In_Urgency_Order_Head --
   -----------------------------------

   --  Go from head to tail. Place E before the first P such as E >= P

   procedure Enqueue_In_Urgency_Order_Head (E : in Element_Ac;
                                            L : in out List) is

       P : Element_Ac := Element_Ac (L);
   begin
      --  special case: first position (empty queue or E >= P)

      if L = null or else not (P > E) then
         L := List (E);
         E.Next := P;
         return;
      end if;

      --  other position different from the first one

      loop
         if P.Next = null or else not (P.Next > E) then
            --  found the place in the queue: after P
      ...
      pragma Assert (False);
   end Enqueue_In_Urgency_Order_Head;
   ...
   begin
      if Is_Empty (ML.PL (Get_Priority (E))) then
         ...
         then
            ...
            Enqueue_In_Urgency_Order_Tail (E, ML.PL (Get_Priority (E)),
                                           FIFO_Order => True);
         end if;
      end if;
   ...
   begin
      ...
      then
         ...
         Enqueue_In_Urgency_Order_Head (E, ML.PL (Get_Priority (E)));
      end if;
