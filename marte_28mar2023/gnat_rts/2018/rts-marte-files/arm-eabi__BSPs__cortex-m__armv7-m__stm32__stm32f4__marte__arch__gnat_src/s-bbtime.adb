...
--  MaRTE OS: modified to be used in the HAL of MaRTE OS for STM32f4* and
--  Micro:bit (Cortex-M processors)
...
--  with System.BB.Timing_Events;  MARTE OS
...
package body System.BB.Time is
   ...
   MaRTE_Alarm_Handler : MaRTE_Handler_Proc_Ac := null;

   procedure Set_MaRTE_Alarm_Handler (Handler : MaRTE_Handler_Proc_Ac) is
   begin
      MaRTE_Alarm_Handler := Handler;
   end Set_MaRTE_Alarm_Handler;
   ...
   begin
      ...
      MaRTE_Alarm_Handler.all; --  MaRTE OS

      --  Execute expired events of the current CPU

      --  Timing_Events.Execute_Expired_Timing_Events (Now); --  MaRTE OS

      --  Wake up our alarms, and set any new alarm

      --  Wakeup_Expired_Alarms (Now); --  MaRTE OS
