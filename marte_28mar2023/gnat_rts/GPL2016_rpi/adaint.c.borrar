...
#else
//#include "config.h" <--- MaRTE OS
//#include "system.h" <--- MaRTE OS
//#include "version.h" <--- MaRTE OS
#endif
...
// << MaRTE 
// Just to avoid a linker error in:
//        libgnat.a(a-sysdep.o): In function `__gnat_ttyname':
#include <stdio.h>
char *ttyname ( int desc )
{
  printe("  INTERNAL ERROR: function ttyname non implemented\n");
  return 0;
}
// MaRTE >>
...
struct stat64 // MaRTE OS (used to define GNAT_STRUCT_STAT in adaint.h)
{
  int foo;
};
...
__gnat_rmdir (char *path)
{
  ...
  || defined (__FreeBSD__)
  //#define HAS_TARGET_WCHAR_T   <-- MaRTE OS
#endif
...
#else
  // return GNAT_FOPEN (path, mode); MaRTE
...
__int64 __gnat_file_length (int fd)
{
  return -1;
}

__int64 __gnat_named_file_length (char *name)
{
  return -1;
}
...
char *
__gnat_readdir (DIR *dirp, char *buffer, int *i)
{
  return NULL;
}
// MaRTE OS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
#if 0    // <---------- MaRTE OS 
// MaRTE OS: this function is not used in s-osinte.lwp_self
