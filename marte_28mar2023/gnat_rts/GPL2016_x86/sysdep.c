...
// << MaRTE OS
//extern void getc_immediate_common (FILE *, int *, int *, int *, int);MaRTE OS
...
void getc_immediate (FILE *stream, int *ch, int *end_of_file) {
  ...
  char mode;
  *end_of_file=0;
  ret = ioctl(fileno(stream), PC_KEYBOARD_SAVE_MODE, &mode);
  ret = ioctl(fileno(stream), PC_KEYBOARD_SET_RAW_MODE, NULL);
  *ch = fgetc(stream);
  ret = ioctl(fileno(stream), PC_KEYBOARD_RESTORE_MODE, &mode);
}
...
void getc_immediate_nowait (FILE *stream, int *ch,
  ...
  *end_of_file = 0;
  ret = ioctl(fileno(stream), PC_KEYBOARD_SAVE_MODE, &mode);
  ret = ioctl(fileno(stream), PC_KEYBOARD_SET_RAW_MODE, NULL);
  ret = ioctl(fileno(stream), RESET_BLOCKING_MODE, NULL);
  *ch = fgetc(stream);
  *avail = *ch != EOF;
  ret = ioctl(fileno(stream), PC_KEYBOARD_RESTORE_MODE, &mode);
} 
...
/* Darwin, Free BSD, Linux, where component tm_gmtoff is present in
  ...
  || defined (__GLIBC__)
{
//  localtime_r (timer, &tp);  MaRTE OS
//  *off = tp.tm_gmtoff;  MaRTE OS
  *off = 0; //  MaRTE OS
}
