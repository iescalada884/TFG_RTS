...
package body System.Tasking.Stages is
   ...
   procedure Activate_Tasks (Chain_Access : Activation_Chain_Access) is
      ...
      Activate_Deadline_Ac : access OSI.timespec;  --  MaRTE OS
      Activate_Deadline : aliased OSI.timespec;  --  MaRTE OS
      Success        : Boolean;
   ...
   begin
      ...
      begin
         --  get initial deadline of activator task

         Result := OSI.pthread_getdeadline (STPO.Get_Thread_Id (Self_ID),
                                            OSI.CLOCK_REALTIME,
                                            Activate_Deadline'Access);
         pragma Assert (Result = 0);

         --  Disable RR quantum during activation

         Result := OSI.marte_disable_rr_quantum (STPO.Get_Thread_Id (Self_ID));
         pragma Assert (Result = 0);
      end;
      ...
      while C /= null loop
         if C.Common.State /= Terminated then
            ...
            --  MaRTE OS: inherit activator deadline (if shorter than task's)

            Activate_Deadline_Ac :=
              (if STPO.Monotonic_Clock + C.Common.Relative_Deadline <
                 OSI.To_Duration (Activate_Deadline)
               then null
               else Activate_Deadline'Access);

            System.Task_Primitives.Operations.Create_Task
              (C, Task_Wrapper'Address,
               Parameters.Size_Type
                 (C.Common.Compiler_Data.Pri_Stack_Info.Size),
               Activate_Prio, Success, Activate_Deadline_Ac);
      ...
      --  MaRTE OS: enable RR quantum (it was disabled during activation)

      declare
         Result : Interfaces.C.int;
         use type Interfaces.C.int;
      begin
         Result := OSI.marte_enable_rr_quantum (STPO.Get_Thread_Id (Self_ID));
         pragma Assert (Result = 0);
      end;
   end Activate_Tasks;
   ...
   is
      ...
      --  pragma Unreferenced (Relative_Deadline);  --  MaRTE OS
   ...
   begin
      ...
      Initialize_ATCB (Self_ID, State, Discriminants, P, Elaborated,
        Base_Priority, Base_CPU, Domain, Task_Info, Size, T, Success,
        Ada.Real_Time.To_Duration (Relative_Deadline));
   ...
   begin
      ...
      if Dispatching_Policy = 'E'
        or else Get_Policy (Self_ID.Common.Base_Priority) = 'E'
      then
         declare
            ...
            use type Interfaces.C.int;
         begin
            --  set the deadline of the task (it could have inherited the
            --  deadline of its activator and here that inheritance finish)
            TS := OSI.To_Timespec (STPO.Monotonic_Clock
                                   + Self_ID.Common.Relative_Deadline);
            Result := OSI.pthread_setdeadline (STPO.Get_Thread_Id (Self_ID),
                                               TS'Access,
                                               OSI.CLOCK_REALTIME,
                                               immediate => 1);
            pragma Assert (Result = 0);
      ...
      if Dispatching_Policy = 'R'
          or else Get_Policy (Self_ID.Common.Base_Priority) = 'R'
      then
         declare
            Result : Interfaces.C.int;
            use type Interfaces.C.int;
         begin
            --  Enable RR quantum (it was disabled during activation)

            Result :=
              OSI.marte_enable_rr_quantum (STPO.Get_Thread_Id (Self_ID));
            pragma Assert (Result = 0);
         end;
      end if;
